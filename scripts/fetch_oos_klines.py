#!/usr/bin/env python3
"""Manual REST backfill for OOS 1m klines.

We copy the existing per-symbol gz (which ends ~2026-04-23) and append freshly
fetched klines through the latest completed minute. Spot from api.binance.com,
futures from fapi.binance.com. Do not run this as the live data loop; live uses
websockets via live/update_live_klines.py.
"""
...
def extend(market: str, base: str, symbol: str, end_ms: int, lookback_hours: float) -> tuple[int, str, str]:
    src = SRC / market / symbol / "1m.csv.gz"
    dst = DST / market / symbol / "1m.csv.gz"
    dst.parent.mkdir(parents=True, exist_ok=True)
    old_path = dst if dst.exists() else src
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manual REST backfill for OOS Binance 1m klines.")
    parser.add_argument(
        "--end",
...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        n1, last1, _ = extend(SPOT, "https://api.binance.com/api/v3/klines", symbol, end_ms, args.lookback_hours)
        n2, last2, _ = extend(FUT, "https://fapi.binance.com/fapi/v1/klines", symbol, end_ms, args.lookback_hours)
