...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Binance live klines, LightGBM signals, and Polymarket trader.")
    ...
    parser.add_argument("--poly-config", type=Path, default=LIVE_DIR / "polymarket" / "poly_config.json")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default production stack is 15m=live/models_15m."
        ),
    )
    parser.add_argument("--dry-run", action="store_true", help="Force Polymarket dry-run mode.")
    ...
    if not args.no_trader:
        trader_command = [
            python,
            str(LIVE_DIR / "polymarket" / "run_poly_live.py"),
            "--config",
            str(args.poly_config),
        ]
        if args.dry_run:
            trader_command.append("--dry-run")
    ...
    try:
        ...
        print(colorize_line("stopping live stack"), flush=True)
        stop_all(processes)
        return 130
