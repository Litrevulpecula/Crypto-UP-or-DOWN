...
"${SSH[@]}" "$VPS" "cd '$REMOTE_DIR' && tmux kill-session -t hibt_trader 2>/dev/null || true"
"${SSH[@]}" "$VPS" "cd '$REMOTE_DIR' && tmux new -ds hibt_trader \". live/hibt/runtime/hibt_env.sh && .venv/bin/python live/hibt/run_hibt_api_trader.py --live --timeframes 3m,5m,15m\""
"${SSH[@]}" "$VPS" "cd '$REMOTE_DIR' && sleep 2 && tmux capture-pane -p -t hibt_trader -S -20"
